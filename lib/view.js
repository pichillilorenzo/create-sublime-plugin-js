...
class View {
  ...
  */
   ...
   */
  add_regions (key /*: string*/, regions /*: Array<Region>*/, scope /*: ?string*/ = '', icon /*: ?string*/ = '', flags /*: ?number*/, step /*: ?StepObject*/) /*: Promise<null>*/ {
    ...
    if (flags)
      code = `${config.variableMappingName}["${this.self.mapTo}"].add_regions(${key}, ${regionsArray}, """${scope}""", """${icon}""", ${flags})`
    else
      code = `${config.variableMappingName}["${this.self.mapTo}"].add_regions(${key}, ${regionsArray}, """${scope}""", """${icon}""")`
   ...
   */
  show_popup_menu (items /*: Array<string>*/, on_done /*: (number) => void*/, flags /*: ?number*/, step /*: ?StepObject*/) /*: void*/ {
    util.callbackPython(`${config.variableMappingName}["${this.self.mapTo}"].show_popup_menu(json.dumps("""${JSON.stringify(items)}""", ensure_ascii=False), lambda: callback($PORT_TOKEN))`, on_done)
  }
