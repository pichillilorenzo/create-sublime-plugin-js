...
class Settings {
  ...
  */
  ...
  add_on_change (key /*: string*/, callback /*: (?StepObject) => void*/) /*: void*/ {
    util.callbackPython(`${config.variableMappingName}["${this.self.mapTo}"].add_on_change("""${key}""", lambda: sublime.set_timeout_async(lambda: callback($PORT_TOKEN)))`, [async (httpTempServers, subStep) => {
